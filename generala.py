...
class Dados:
    ...
    def realizar_tirada(self):                                                       #* Devuelve una lista con 5 numeros al azar comprendidos entre 1 y 6
        for i in range(self.numero):
            self.dado = random.randrange(1,7)
            self.dados.append(self.dado)
            self.dados.sort()
        return self.dados
    
    def apartar_dados(self):                                                         #* Verifica los repetidos y los aparta para el usuario, de esta forma se realizan jugadas armadas                       
    ...
    def ordenar_repetidos(self):                                                     #* Solución a un bug, en el que las listas con 3 o mas valores repetidos duplicaban su longitud
        if len(self.repetidos) == 16:                                                
            for pos in range(8):
    ...
    def sumar_dados(self):                                                           #* Suma el puntaje total de la jugada
        self.puntaje_c = sum(self.dados)

    def determinar_categoria(self):                                                  #* Determina la categoria de la tirada final en base al numero repetido de dados
        self.contador_1 = 1
        self.contador_2 = 0
        for h in range(4):
...
class Tabla:
    ...
    def generar_tabla(self,jugadores):                                 #* Genera la tabla inicial con los jugadores y sus puntajes en una lista (lista vacia en un principio)
    ...
    def iniciar_juego(self,jugadores,dados):                                             #* Realiza una tirada y ordena a los jugadores en orden decreciente de puntaje, para determinar el orden de los turnos
    ...
    def tabla_final(self,jugadores):                                                                                #* Asigna las posiciones finales y genera la tabla final en el order de los dados tirados anteriormente
        if len(jugadores.jugadores) == 2:    
    ...
    def turnos(self):                                                                                #* Inicia los turnos
        ...
        for turn in range(3):                                                                        #* Hay un total de 3 turnos
            ...
            for jug in range(len(jugadores.jugadores)):                                              #* El turno se repite para la cantidad de jugadores
                ...
                dados.repetidos = []
                self.dados_final = []
                dados.numero = 5                                                                      #* Determina la cantidad inicial de dados, la cantidad cambia si se conservan dados en una tirada armada
                print('\n','-'*42)
                ...
                if len(dados.repetidos) != 0 and len(dados.repetidos) < 5:                            #* En el caso de haber conservado dados, vuelve a lanzar hasta un maximo de 2 veces mas con los dados sobrantes
                ...
                if dados.cat == 'Sin categoria':                                 #* Se suman los puntajes individuales en base a la categoria, si es generala, el turno se termina
                    self.sc += dados.puntaje_c                                   #* La categoria esta programada de forma en la que si bien sacar determinada categoria aumenta la posibilidad de ganar con generala, esta realmente no afecta el resultado final de victoria
            ...
            print(tabla.tabla)
            print('-'*33)
            time.sleep(2)                                                                   #* Tiempo de espera para que el usuario pueda leer los resultados
